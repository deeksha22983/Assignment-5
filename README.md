...
*   **Limitation of Decision Trees:** A primary limitation is the high tendency to overfit the training data. Decision Trees often build overly complex branches to memorize the exact training set, resulting in poor generalization on unseen data.
*   **Limitation of Random Forest:** The main limitation is a loss of interpretability. It acts as a "black box" model; unlike a simple Decision Tree where HR managers can visually trace the exact logical path that led to a prediction, a Random Forest's decision-making process involving hundreds of trees is too complex to intuitively explain.
